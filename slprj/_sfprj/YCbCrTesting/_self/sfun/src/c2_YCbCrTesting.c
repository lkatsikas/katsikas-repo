...
static void initialize_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void initialize_params_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void enable_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void disable_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c2_YCbCrTesting
  (SFc2_YCbCrTestingInstanceStruct *chartInstance);
static void set_sim_state_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c2_st);
static void finalize_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void sf_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct *chartInstance);
static void c2_chartstep_c2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void initSimStructsc2_YCbCrTesting(SFc2_YCbCrTestingInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c2_machineNumber, uint32_T
  c2_chartNumber);
static void c2_info_helper(c2_ResolvedFunctionInfo c2_info[16]);
static void c2_eml_error(SFc2_YCbCrTestingInstanceStruct *chartInstance);
static void c2_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c2_out, const char_T *c2_identifier, uint32_T c2_y[20]);
static void c2_b_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId, uint32_T c2_y[20]);
static void c2_c_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c2_b_prev_out, const char_T *c2_identifier, uint32_T c2_y[20]);
static void c2_d_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId, uint32_T c2_y[20]);
static uint8_T c2_e_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c2_b_is_active_c2_YCbCrTesting, const char_T
  *c2_identifier);
static uint8_T c2_f_emlrt_marshallIn(SFc2_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void c2_putbits(SFc2_YCbCrTestingInstanceStruct *chartInstance, int32_T
...
static void init_dsm_address_info(SFc2_YCbCrTestingInstanceStruct *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c2_is_active_c2_YCbCrTesting = 0U;
  ...
  (SFc2_YCbCrTestingInstanceStruct *chartInstance)
{
  ...
  c2_c_u = chartInstance->c2_is_active_c2_YCbCrTesting;
  c2_d_y = NULL;
  ...
  *chartInstance, const mxArray *c2_st)
{
  ...
  chartInstance->c2_is_active_c2_YCbCrTesting = c2_e_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c2_u, 2)),
     "is_active_c2_YCbCrTesting");
  sf_mex_destroy(&c2_u);
...
{
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  c2_chartstep_c2_YCbCrTesting(chartInstance);
}
  ...
  *chartInstance)
{
  ...
  do {
    c2_exitg1 = 0U;
    if (c2_bitsIn <= 64) {
      c2_x = c2_in[c2_bitsIn - 1] + 0.5;
      c2_cval = (real32_T)muDoubleScalarFloor(c2_x);
      c2_guard1 = FALSE;
...
const mxArray *sf_c2_YCbCrTesting_get_eml_resolved_functions_info(void)
{
  ...
  *c2_identifier)
{
  ...
  c2_y = c2_f_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c2_b_is_active_c2_YCbCrTesting), &c2_thisId);
  sf_mex_destroy(&c2_b_is_active_c2_YCbCrTesting);
  return c2_y;
}
...
/* SFunction Glue Code */
void sf_c2_YCbCrTesting_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_YCbCrTesting_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c2_YCbCrTesting(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[264 272],M[0],}}},{M[8],M[0],T\"is_active_c2_YCbCrTesting\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_YCbCrTesting_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c2_YCbCrTesting(void *chartInstanceVar)
{
  initialize_params_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
  initialize_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c2_YCbCrTesting(void *chartInstanceVar)
{
  enable_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c2_YCbCrTesting(void *chartInstanceVar)
{
  disable_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c2_YCbCrTesting(void *chartInstanceVar)
{
  sf_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c2_YCbCrTesting(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c2_YCbCrTesting
    ((SFc2_YCbCrTestingInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c2_YCbCrTesting();/* state var info */
...
extern void sf_internal_set_sim_state_c2_YCbCrTesting(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c2_YCbCrTesting(SimStruct* S)
{
  return sf_internal_get_sim_state_c2_YCbCrTesting(S);
}

static void sf_opaque_set_sim_state_c2_YCbCrTesting(SimStruct* S, const mxArray *
  st)
{
  sf_internal_set_sim_state_c2_YCbCrTesting(S, st);
}

static void sf_opaque_terminate_c2_YCbCrTesting(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc2_YCbCrTesting((SFc2_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_YCbCrTesting(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"YCbCrTesting","YCbCrTesting",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"YCbCrTesting","YCbCrTesting",2,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"YCbCrTesting",
      "YCbCrTesting",2,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"YCbCrTesting","YCbCrTesting",2,1);
      sf_mark_chart_reusable_outputs(S,"YCbCrTesting","YCbCrTesting",2,1);
    }

    sf_set_rtw_dwork_info(S,"YCbCrTesting","YCbCrTesting",2);
...
static void mdlRTW_c2_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "YCbCrTesting", "YCbCrTesting",2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_YCbCrTesting(SimStruct *S)
{
  SFc2_YCbCrTestingInstanceStruct *chartInstance;
  chartInstance = (SFc2_YCbCrTestingInstanceStruct *)malloc(sizeof
    (SFc2_YCbCrTestingInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc2_YCbCrTestingInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c2_YCbCrTesting;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c2_YCbCrTesting;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c2_YCbCrTesting;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c2_YCbCrTesting;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c2_YCbCrTesting;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c2_YCbCrTesting;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c2_YCbCrTesting;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c2_YCbCrTesting;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c2_YCbCrTesting;
  chartInstance->chartInfo.mdlStart = mdlStart_c2_YCbCrTesting;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_YCbCrTesting;
...
void c2_YCbCrTesting_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_YCbCrTesting(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_YCbCrTesting_method_dispatcher.\n"
