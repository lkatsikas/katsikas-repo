...
static void initialize_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static void initialize_params_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static void enable_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static void disable_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c4_YCbCrTesting
  (SFc4_YCbCrTestingInstanceStruct *chartInstance);
static void set_sim_state_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c4_st);
static void finalize_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static void sf_c4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct *chartInstance);
static void initSimStructsc4_YCbCrTesting(SFc4_YCbCrTestingInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c4_machineNumber, uint32_T
  c4_chartNumber);
static void c4_info_helper(c4_ResolvedFunctionInfo c4_info[17]);
static void c4_getbits(SFc4_YCbCrTestingInstanceStruct *chartInstance, int32_T
  c4_cbits, real_T c4_inpos, uint32_T c4_Temp, uint32_T c4_in[20], uint32_T
  c4_val, real_T c4_bits, real_T *c4_flag, int32_T *c4_cbitsO, real_T *c4_inposO,
  uint32_T *c4_TempO, uint32_T *c4_valO);
static void c4_eml_error(SFc4_YCbCrTestingInstanceStruct *chartInstance);
static void c4_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c4_out, const char_T *c4_identifier, real32_T c4_y[64]);
static void c4_b_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId, real32_T c4_y[64]);
static void c4_c_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c4_b_prev_out, const char_T *c4_identifier, real32_T c4_y[64]);
static void c4_d_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId, real32_T c4_y[64]);
static uint8_T c4_e_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c4_b_is_active_c4_YCbCrTesting, const char_T
  *c4_identifier);
static uint8_T c4_f_emlrt_marshallIn(SFc4_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void init_dsm_address_info(SFc4_YCbCrTestingInstanceStruct *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c4_is_active_c4_YCbCrTesting = 0U;
  ...
  (SFc4_YCbCrTestingInstanceStruct *chartInstance)
{
  ...
  c4_c_u = chartInstance->c4_is_active_c4_YCbCrTesting;
  c4_d_y = NULL;
  ...
  *chartInstance, const mxArray *c4_st)
{
  ...
  chartInstance->c4_is_active_c4_YCbCrTesting = c4_e_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c4_u, 2)),
     "is_active_c4_YCbCrTesting");
  sf_mex_destroy(&c4_u);
...
const mxArray *sf_c4_YCbCrTesting_get_eml_resolved_functions_info(void)
{
  ...
  *c4_identifier)
{
  ...
  c4_y = c4_f_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c4_b_is_active_c4_YCbCrTesting), &c4_thisId);
  sf_mex_destroy(&c4_b_is_active_c4_YCbCrTesting);
  return c4_y;
}
...
/* SFunction Glue Code */
void sf_c4_YCbCrTesting_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c4_YCbCrTesting_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c4_YCbCrTesting(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[188 196],M[0],}}},{M[8],M[0],T\"is_active_c4_YCbCrTesting\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c4_YCbCrTesting_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c4_YCbCrTesting(void *chartInstanceVar)
{
  initialize_params_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
  initialize_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c4_YCbCrTesting(void *chartInstanceVar)
{
  enable_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c4_YCbCrTesting(void *chartInstanceVar)
{
  disable_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c4_YCbCrTesting(void *chartInstanceVar)
{
  sf_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c4_YCbCrTesting(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c4_YCbCrTesting
    ((SFc4_YCbCrTestingInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c4_YCbCrTesting();/* state var info */
...
extern void sf_internal_set_sim_state_c4_YCbCrTesting(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c4_YCbCrTesting(SimStruct* S)
{
  return sf_internal_get_sim_state_c4_YCbCrTesting(S);
}

static void sf_opaque_set_sim_state_c4_YCbCrTesting(SimStruct* S, const mxArray *
  st)
{
  sf_internal_set_sim_state_c4_YCbCrTesting(S, st);
}

static void sf_opaque_terminate_c4_YCbCrTesting(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc4_YCbCrTesting((SFc4_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c4_YCbCrTesting(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c4_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"YCbCrTesting","YCbCrTesting",4);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"YCbCrTesting","YCbCrTesting",4,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"YCbCrTesting",
      "YCbCrTesting",4,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"YCbCrTesting","YCbCrTesting",4,1);
      sf_mark_chart_reusable_outputs(S,"YCbCrTesting","YCbCrTesting",4,1);
    }

    sf_set_rtw_dwork_info(S,"YCbCrTesting","YCbCrTesting",4);
...
static void mdlRTW_c4_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "YCbCrTesting", "YCbCrTesting",4);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c4_YCbCrTesting(SimStruct *S)
{
  SFc4_YCbCrTestingInstanceStruct *chartInstance;
  chartInstance = (SFc4_YCbCrTestingInstanceStruct *)malloc(sizeof
    (SFc4_YCbCrTestingInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc4_YCbCrTestingInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c4_YCbCrTesting;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c4_YCbCrTesting;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c4_YCbCrTesting;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c4_YCbCrTesting;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c4_YCbCrTesting;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c4_YCbCrTesting;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c4_YCbCrTesting;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c4_YCbCrTesting;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c4_YCbCrTesting;
  chartInstance->chartInfo.mdlStart = mdlStart_c4_YCbCrTesting;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c4_YCbCrTesting;
...
void c4_YCbCrTesting_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c4_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c4_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c4_YCbCrTesting(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c4_YCbCrTesting_method_dispatcher.\n"
