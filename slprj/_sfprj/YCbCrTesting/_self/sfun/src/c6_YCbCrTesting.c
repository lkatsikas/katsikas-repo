...
static void initialize_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static void initialize_params_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static void enable_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static void disable_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c6_YCbCrTesting
  (SFc6_YCbCrTestingInstanceStruct *chartInstance);
static void set_sim_state_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c6_st);
static void finalize_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static void sf_c6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct *chartInstance);
static void initSimStructsc6_YCbCrTesting(SFc6_YCbCrTestingInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c6_machineNumber, uint32_T
  c6_chartNumber);
static void c6_info_helper(c6_ResolvedFunctionInfo c6_info[17]);
static void c6_getbits(SFc6_YCbCrTestingInstanceStruct *chartInstance, int32_T
  c6_cbits, real_T c6_inpos, uint32_T c6_Temp, uint32_T c6_in[20], uint32_T
  c6_val, real_T c6_bits, real_T *c6_flag, int32_T *c6_cbitsO, real_T *c6_inposO,
  uint32_T *c6_TempO, uint32_T *c6_valO);
static void c6_eml_error(SFc6_YCbCrTestingInstanceStruct *chartInstance);
static void c6_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c6_out, const char_T *c6_identifier, real32_T c6_y[64]);
static void c6_b_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId, real32_T c6_y[64]);
static void c6_c_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c6_b_prev_out, const char_T *c6_identifier, real32_T c6_y[64]);
static void c6_d_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct *chartInstance,
  const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId, real32_T c6_y[64]);
static uint8_T c6_e_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c6_b_is_active_c6_YCbCrTesting, const char_T
  *c6_identifier);
static uint8_T c6_f_emlrt_marshallIn(SFc6_YCbCrTestingInstanceStruct
  *chartInstance, const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId);
static void init_dsm_address_info(SFc6_YCbCrTestingInstanceStruct *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c6_is_active_c6_YCbCrTesting = 0U;
  ...
  (SFc6_YCbCrTestingInstanceStruct *chartInstance)
{
  ...
  c6_c_u = chartInstance->c6_is_active_c6_YCbCrTesting;
  c6_d_y = NULL;
  ...
  *chartInstance, const mxArray *c6_st)
{
  ...
  chartInstance->c6_is_active_c6_YCbCrTesting = c6_e_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c6_u, 2)),
     "is_active_c6_YCbCrTesting");
  sf_mex_destroy(&c6_u);
...
const mxArray *sf_c6_YCbCrTesting_get_eml_resolved_functions_info(void)
{
  ...
  *c6_identifier)
{
  ...
  c6_y = c6_f_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c6_b_is_active_c6_YCbCrTesting), &c6_thisId);
  sf_mex_destroy(&c6_b_is_active_c6_YCbCrTesting);
  return c6_y;
}
...
/* SFunction Glue Code */
void sf_c6_YCbCrTesting_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c6_YCbCrTesting_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c6_YCbCrTesting(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[188 196],M[0],}}},{M[8],M[0],T\"is_active_c6_YCbCrTesting\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c6_YCbCrTesting_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c6_YCbCrTesting(void *chartInstanceVar)
{
  initialize_params_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
  initialize_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c6_YCbCrTesting(void *chartInstanceVar)
{
  enable_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c6_YCbCrTesting(void *chartInstanceVar)
{
  disable_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c6_YCbCrTesting(void *chartInstanceVar)
{
  sf_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c6_YCbCrTesting(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c6_YCbCrTesting
    ((SFc6_YCbCrTestingInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c6_YCbCrTesting();/* state var info */
...
extern void sf_internal_set_sim_state_c6_YCbCrTesting(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c6_YCbCrTesting(SimStruct* S)
{
  return sf_internal_get_sim_state_c6_YCbCrTesting(S);
}

static void sf_opaque_set_sim_state_c6_YCbCrTesting(SimStruct* S, const mxArray *
  st)
{
  sf_internal_set_sim_state_c6_YCbCrTesting(S, st);
}

static void sf_opaque_terminate_c6_YCbCrTesting(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc6_YCbCrTesting((SFc6_YCbCrTestingInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c6_YCbCrTesting(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c6_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"YCbCrTesting","YCbCrTesting",6);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"YCbCrTesting","YCbCrTesting",6,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"YCbCrTesting",
      "YCbCrTesting",6,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"YCbCrTesting","YCbCrTesting",6,1);
      sf_mark_chart_reusable_outputs(S,"YCbCrTesting","YCbCrTesting",6,1);
    }

    sf_set_rtw_dwork_info(S,"YCbCrTesting","YCbCrTesting",6);
...
static void mdlRTW_c6_YCbCrTesting(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "YCbCrTesting", "YCbCrTesting",6);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c6_YCbCrTesting(SimStruct *S)
{
  SFc6_YCbCrTestingInstanceStruct *chartInstance;
  chartInstance = (SFc6_YCbCrTestingInstanceStruct *)malloc(sizeof
    (SFc6_YCbCrTestingInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc6_YCbCrTestingInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c6_YCbCrTesting;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c6_YCbCrTesting;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c6_YCbCrTesting;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c6_YCbCrTesting;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c6_YCbCrTesting;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c6_YCbCrTesting;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c6_YCbCrTesting;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c6_YCbCrTesting;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c6_YCbCrTesting;
  chartInstance->chartInfo.mdlStart = mdlStart_c6_YCbCrTesting;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c6_YCbCrTesting;
...
void c6_YCbCrTesting_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c6_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c6_YCbCrTesting(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c6_YCbCrTesting(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c6_YCbCrTesting_method_dispatcher.\n"
