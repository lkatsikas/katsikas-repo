...
static void initialize_c6_video(SFc6_videoInstanceStruct *chartInstance);
static void initialize_params_c6_video(SFc6_videoInstanceStruct *chartInstance);
static void enable_c6_video(SFc6_videoInstanceStruct *chartInstance);
static void disable_c6_video(SFc6_videoInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c6_video(SFc6_videoInstanceStruct
  *chartInstance);
static void set_sim_state_c6_video(SFc6_videoInstanceStruct *chartInstance,
  const mxArray *c6_st);
static void finalize_c6_video(SFc6_videoInstanceStruct *chartInstance);
static void sf_c6_video(SFc6_videoInstanceStruct *chartInstance);
static void initSimStructsc6_video(SFc6_videoInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c6_machineNumber, uint32_T
  c6_chartNumber);
static void c6_info_helper(c6_ResolvedFunctionInfo c6_info[17]);
static void c6_getbits(SFc6_videoInstanceStruct *chartInstance, int32_T c6_cbits,
...
static void c6_eml_error(SFc6_videoInstanceStruct *chartInstance);
static void c6_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance, const
  mxArray *c6_out, const char_T *c6_identifier, real32_T c6_y[64]);
static void c6_b_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance, const
  mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId, real32_T c6_y[64]);
static void c6_c_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance, const
  mxArray *c6_b_prev_out, const char_T *c6_identifier, real32_T c6_y[64]);
static void c6_d_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance, const
  mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId, real32_T c6_y[64]);
static uint8_T c6_e_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance,
  const mxArray *c6_b_is_active_c6_video, const char_T *c6_identifier);
static uint8_T c6_f_emlrt_marshallIn(SFc6_videoInstanceStruct *chartInstance,
  const mxArray *c6_u, const emlrtMsgIdentifier *c6_parentId);
static void init_dsm_address_info(SFc6_videoInstanceStruct *chartInstance);
...
static void initialize_c6_video(SFc6_videoInstanceStruct *chartInstance)
{
  ...
  chartInstance->c6_is_active_c6_video = 0U;
  ...
  *chartInstance)
{
  ...
  c6_c_u = chartInstance->c6_is_active_c6_video;
  c6_d_y = NULL;
  ...
  const mxArray *c6_st)
{
  ...
  chartInstance->c6_is_active_c6_video = c6_e_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c6_u, 2)), "is_active_c6_video");
  sf_mex_destroy(&c6_u);
...
const mxArray *sf_c6_video_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c6_b_is_active_c6_video, const char_T *c6_identifier)
{
  ...
  c6_thisId.fIdentifier = c6_identifier;
  c6_thisId.fParent = NULL;
  c6_y = c6_f_emlrt_marshallIn(chartInstance, sf_mex_dup(c6_b_is_active_c6_video),
    &c6_thisId);
  sf_mex_destroy(&c6_b_is_active_c6_video);
  return c6_y;
}
...
/* SFunction Glue Code */
void sf_c6_video_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c6_video_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c6_video(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[188 196],M[0],}}},{M[8],M[0],T\"is_active_c6_video\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c6_video_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c6_video(void *chartInstanceVar)
{
  initialize_params_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
  initialize_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c6_video(void *chartInstanceVar)
{
  enable_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c6_video(void *chartInstanceVar)
{
  disable_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c6_video(void *chartInstanceVar)
{
  sf_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c6_video(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c6_video((SFc6_videoInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c6_video();/* state var info */
...
extern void sf_internal_set_sim_state_c6_video(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c6_video((SFc6_videoInstanceStruct*)chartInfo->chartInstance,
    mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c6_video(SimStruct* S)
{
  return sf_internal_get_sim_state_c6_video(S);
}

static void sf_opaque_set_sim_state_c6_video(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c6_video(S, st);
}

static void sf_opaque_terminate_c6_video(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc6_videoInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc6_video((SFc6_videoInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c6_video(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c6_video((SFc6_videoInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c6_video(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"video","video",6);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"video","video",6,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"video","video",6,
      "gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"video","video",6,1);
      sf_mark_chart_reusable_outputs(S,"video","video",6,1);
    }

    sf_set_rtw_dwork_info(S,"video","video",6);
...
static void mdlRTW_c6_video(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "video", "video",6);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c6_video(SimStruct *S)
{
  SFc6_videoInstanceStruct *chartInstance;
  chartInstance = (SFc6_videoInstanceStruct *)malloc(sizeof
    (SFc6_videoInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc6_videoInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c6_video;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c6_video;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c6_video;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c6_video;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c6_video;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c6_video;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c6_video;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c6_video;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c6_video;
  chartInstance->chartInfo.mdlStart = mdlStart_c6_video;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c6_video;
...
void c6_video_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   ...
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c6_video(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c6_video(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c6_video_method_dispatcher.\n"
