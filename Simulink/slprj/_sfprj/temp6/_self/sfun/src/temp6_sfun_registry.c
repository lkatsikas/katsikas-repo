...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_temp6_process_check_sum_call( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_temp6_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_temp6_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "temp6") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_temp6_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_temp6_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "temp6") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_temp6_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_temp6_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"temp6")) {
    return(sf_temp6_method_dispatcher(simstructPtr,chartFileNumber,specsCksum,
            method,data));
  }
...
extern void temp6_terminator(void);
...
extern void temp6_initializer(void);
extern void temp6_register_exported_symbols(SimStruct* S);
extern void temp6_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  temp6_register_exported_symbols(S);
}
