...
static void initialize_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void initialize_params_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void enable_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void disable_c2_video(SFc2_videoInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c2_video(SFc2_videoInstanceStruct
  *chartInstance);
static void set_sim_state_c2_video(SFc2_videoInstanceStruct *chartInstance,
  const mxArray *c2_st);
static void finalize_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void sf_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void c2_chartstep_c2_video(SFc2_videoInstanceStruct *chartInstance);
static void initSimStructsc2_video(SFc2_videoInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c2_machineNumber, uint32_T
  c2_chartNumber);
static void c2_info_helper(c2_ResolvedFunctionInfo c2_info[16]);
static void c2_eml_error(SFc2_videoInstanceStruct *chartInstance);
static void c2_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance, const
  mxArray *c2_out, const char_T *c2_identifier, uint32_T c2_y[20]);
static void c2_b_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance, const
  mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId, uint32_T c2_y[20]);
static void c2_c_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance, const
  mxArray *c2_b_prev_out, const char_T *c2_identifier, uint32_T c2_y[20]);
static void c2_d_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance, const
  mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId, uint32_T c2_y[20]);
static uint8_T c2_e_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance,
  const mxArray *c2_b_is_active_c2_video, const char_T *c2_identifier);
static uint8_T c2_f_emlrt_marshallIn(SFc2_videoInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void c2_putbits(SFc2_videoInstanceStruct *chartInstance, int32_T
...
static void init_dsm_address_info(SFc2_videoInstanceStruct *chartInstance);
...
static void initialize_c2_video(SFc2_videoInstanceStruct *chartInstance)
{
  ...
  chartInstance->c2_is_active_c2_video = 0U;
  ...
  *chartInstance)
{
  ...
  c2_c_u = chartInstance->c2_is_active_c2_video;
  c2_d_y = NULL;
  ...
  const mxArray *c2_st)
{
  ...
  chartInstance->c2_is_active_c2_video = c2_e_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c2_u, 2)), "is_active_c2_video");
  sf_mex_destroy(&c2_u);
...
{
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  c2_chartstep_c2_video(chartInstance);
}
...
static void c2_chartstep_c2_video(SFc2_videoInstanceStruct *chartInstance)
{
  ...
  if ((real_T)(*c2_b_in)[0] > 0.0) {
    c2_cval = muSingleScalarFloor((*c2_b_in)[0]);
  } else {
    c2_cval = muSingleScalarCeil((*c2_b_in)[0]);
  }

  if ((c2_cval < 8.388608E+6F) && (c2_cval > -8.388608E+6F)) {
    c2_cval = (real_T)c2_cval < 0.0 ? muSingleScalarCeil(c2_cval - 0.5F) :
      muSingleScalarFloor(c2_cval + 0.5F);
  }

  if (c2_cval < 2.14748365E+9F) {
    if (c2_cval >= -2.14748365E+9F) {
      c2_out1 = (int32_T)c2_cval;
    } else {
      c2_out1 = MIN_int32_T;
    }
  } else if (c2_cval >= 2.14748365E+9F) {
    c2_out1 = MAX_int32_T;
  } else {
    c2_out1 = 0;
  }
  ...
  c2_cval = (*c2_b_in)[1] + 0.5F;
  if ((real_T)c2_cval > 0.0) {
    c2_pval = muSingleScalarFloor(c2_cval);
  } else {
    c2_pval = muSingleScalarCeil(c2_cval);
  }
  ...
  do {
    c2_exitg1 = 0U;
    if (c2_bitsIn <= 64) {
      c2_cval = c2_in[c2_bitsIn - 1] + 0.5F;
      c2_cval = muSingleScalarFloor(c2_cval);
      c2_guard1 = FALSE;
      if ((c2_pval == c2_cval) && (c2_out1 < 14)) {
        ...
        c2_putbits(chartInstance, c2_bitpos, c2_out, c2_Temp, c2_outpos,
                   (uint32_T)c2_out1, 4.0, &c2_flag, &c2_b_bitpos, &c2_b_Temp,
                   &c2_b_outpos);
        if (!(c2_flag != 0.0)) {
          for (c2_out1 = 0; c2_out1 < 20; c2_out1++) {
          ...
          c2_putbits(chartInstance, c2_b_bitpos, c2_out, c2_b_Temp, c2_b_outpos,
                     c2_valIn, 8.0, &c2_flag, &c2_bitpos, &c2_Temp, &c2_outpos);
          if (!(c2_flag != 0.0)) {
            for (c2_out1 = 0; c2_out1 < 20; c2_out1++) {
              chartInstance->c2_prev_out[c2_out1] = c2_out[c2_out1];
            }
      ...
      c2_putbits(chartInstance, c2_bitpos, c2_out, c2_Temp, c2_outpos, (uint32_T)
                 c2_out1, 4.0, &c2_flag, &c2_b_bitpos, &c2_b_Temp, &c2_b_outpos);
      if (!(c2_flag != 0.0)) {
        for (c2_out1 = 0; c2_out1 < 20; c2_out1++) {
          chartInstance->c2_prev_out[c2_out1] = c2_out[c2_out1];
        }
...
const mxArray *sf_c2_video_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c2_b_is_active_c2_video, const char_T *c2_identifier)
{
  ...
  c2_thisId.fIdentifier = c2_identifier;
  c2_thisId.fParent = NULL;
  c2_y = c2_f_emlrt_marshallIn(chartInstance, sf_mex_dup(c2_b_is_active_c2_video),
    &c2_thisId);
  sf_mex_destroy(&c2_b_is_active_c2_video);
  return c2_y;
}
...
/* SFunction Glue Code */
void sf_c2_video_get_check_sum(mxArray *plhs[])
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(2089426950U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(1255853625U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(815242461U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(2125536275U);
}

mxArray *sf_c2_video_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c2_video(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[264 272],M[0],}}},{M[8],M[0],T\"is_active_c2_video\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_video_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c2_video(void *chartInstanceVar)
{
  initialize_params_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
  initialize_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c2_video(void *chartInstanceVar)
{
  enable_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c2_video(void *chartInstanceVar)
{
  disable_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c2_video(void *chartInstanceVar)
{
  sf_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c2_video(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c2_video((SFc2_videoInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c2_video();/* state var info */
...
extern void sf_internal_set_sim_state_c2_video(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c2_video((SFc2_videoInstanceStruct*)chartInfo->chartInstance,
    mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c2_video(SimStruct* S)
{
  return sf_internal_get_sim_state_c2_video(S);
}

static void sf_opaque_set_sim_state_c2_video(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c2_video(S, st);
}

static void sf_opaque_terminate_c2_video(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc2_videoInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc2_video((SFc2_videoInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_video(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c2_video((SFc2_videoInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c2_video(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"video","video",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"video","video",2,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"video","video",2,
      "gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"video","video",2,1);
      sf_mark_chart_reusable_outputs(S,"video","video",2,1);
    }

    sf_set_rtw_dwork_info(S,"video","video",2);
    ssSetHasSubFunctions(S,!(chartIsInlinable));
  } else {
  }

  ssSetOptions(S,ssGetOptions(S)|SS_OPTION_WORKS_WITH_CODE_REUSE);
  ssSetChecksum0(S,(3047363482U));
  ssSetChecksum1(S,(1622713879U));
  ssSetChecksum2(S,(3981071900U));
  ssSetChecksum3(S,(664979475U));
  ssSetmdlDerivatives(S, NULL);
  ssSetExplicitFCSSCtrl(S,1);
}

static void mdlRTW_c2_video(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "video", "video",2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_video(SimStruct *S)
{
  SFc2_videoInstanceStruct *chartInstance;
  chartInstance = (SFc2_videoInstanceStruct *)malloc(sizeof
    (SFc2_videoInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc2_videoInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c2_video;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c2_video;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c2_video;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c2_video;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c2_video;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c2_video;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c2_video;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c2_video;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c2_video;
  chartInstance->chartInfo.mdlStart = mdlStart_c2_video;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_video;
...
void c2_video_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   ...
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_video(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_video(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_video_method_dispatcher.\n"
