...
static void initialize_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static void initialize_params_c5_MPEGConversion
  (SFc5_MPEGConversionInstanceStruct *chartInstance);
static void enable_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static void disable_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c5_MPEGConversion
  (SFc5_MPEGConversionInstanceStruct *chartInstance);
static void set_sim_state_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_st);
static void finalize_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static void sf_c5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static void initSimStructsc5_MPEGConversion(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c5_machineNumber, uint32_T
  c5_chartNumber);
static void c5_info_helper(c5_ResolvedFunctionInfo c5_info[17]);
static void c5_getbits(SFc5_MPEGConversionInstanceStruct *chartInstance, int32_T
  c5_cbits, real_T c5_inpos, uint32_T c5_Temp, uint32_T c5_in[20], uint32_T
  c5_val, real_T c5_bits, real_T *c5_flag, int32_T *c5_cbitsO, real_T *c5_inposO,
  uint32_T *c5_TempO, uint32_T *c5_valO);
static void c5_eml_error(SFc5_MPEGConversionInstanceStruct *chartInstance);
static void c5_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct *chartInstance,
  const mxArray *c5_out, const char_T *c5_identifier, real32_T c5_y[64]);
static void c5_b_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_u, const emlrtMsgIdentifier *c5_parentId,
  real32_T c5_y[64]);
static void c5_c_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_b_prev_out, const char_T *c5_identifier,
  real32_T c5_y[64]);
static void c5_d_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_u, const emlrtMsgIdentifier *c5_parentId,
  real32_T c5_y[64]);
static uint8_T c5_e_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_b_is_active_c5_MPEGConversion, const char_T *
  c5_identifier);
static uint8_T c5_f_emlrt_marshallIn(SFc5_MPEGConversionInstanceStruct
  *chartInstance, const mxArray *c5_u, const emlrtMsgIdentifier *c5_parentId);
static void init_dsm_address_info(SFc5_MPEGConversionInstanceStruct
  *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c5_is_active_c5_MPEGConversion = 0U;
  ...
  (SFc5_MPEGConversionInstanceStruct *chartInstance)
{
  ...
  c5_c_u = chartInstance->c5_is_active_c5_MPEGConversion;
  c5_d_y = NULL;
  ...
  *chartInstance, const mxArray *c5_st)
{
  ...
  chartInstance->c5_is_active_c5_MPEGConversion = c5_e_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c5_u, 2)),
     "is_active_c5_MPEGConversion");
  sf_mex_destroy(&c5_u);
...
const mxArray *sf_c5_MPEGConversion_get_eml_resolved_functions_info(void)
{
  ...
  c5_identifier)
{
  ...
  c5_y = c5_f_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c5_b_is_active_c5_MPEGConversion), &c5_thisId);
  sf_mex_destroy(&c5_b_is_active_c5_MPEGConversion);
  return c5_y;
}
...
/* SFunction Glue Code */
void sf_c5_MPEGConversion_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c5_MPEGConversion_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c5_MPEGConversion(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[5],T\"out\",},{M[4],M[0],T\"prev_out\",S'l','i','p'{{M1x2[188 196],M[0],}}},{M[8],M[0],T\"is_active_c5_MPEGConversion\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c5_MPEGConversion_get_check_sum(&mxChecksum);
  mxSetField(mxInfo, 0, infoFields[0], mxChecksum);
  mxSetField(mxInfo, 0, infoFields[1], mxVarInfo);
  return mxInfo;
}

static void sf_opaque_initialize_c5_MPEGConversion(void *chartInstanceVar)
{
  initialize_params_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
    chartInstanceVar);
  initialize_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c5_MPEGConversion(void *chartInstanceVar)
{
  enable_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c5_MPEGConversion(void *chartInstanceVar)
{
  disable_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c5_MPEGConversion(void *chartInstanceVar)
{
  sf_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c5_MPEGConversion(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c5_MPEGConversion
    ((SFc5_MPEGConversionInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c5_MPEGConversion();/* state var info */
...
extern void sf_internal_set_sim_state_c5_MPEGConversion(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c5_MPEGConversion(SimStruct* S)
{
  return sf_internal_get_sim_state_c5_MPEGConversion(S);
}

static void sf_opaque_set_sim_state_c5_MPEGConversion(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c5_MPEGConversion(S, st);
}

static void sf_opaque_terminate_c5_MPEGConversion(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc5_MPEGConversionInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc5_MPEGConversion((SFc5_MPEGConversionInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c5_MPEGConversion(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c5_MPEGConversion(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"MPEGConversion","MPEGConversion",5);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"MPEGConversion","MPEGConversion",5,
                "RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"MPEGConversion",
      "MPEGConversion",5,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"MPEGConversion","MPEGConversion",5,
        1);
      sf_mark_chart_reusable_outputs(S,"MPEGConversion","MPEGConversion",5,1);
    }

    sf_set_rtw_dwork_info(S,"MPEGConversion","MPEGConversion",5);
...
static void mdlRTW_c5_MPEGConversion(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "MPEGConversion", "MPEGConversion",5);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c5_MPEGConversion(SimStruct *S)
{
  SFc5_MPEGConversionInstanceStruct *chartInstance;
  chartInstance = (SFc5_MPEGConversionInstanceStruct *)malloc(sizeof
    (SFc5_MPEGConversionInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc5_MPEGConversionInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c5_MPEGConversion;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c5_MPEGConversion;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c5_MPEGConversion;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c5_MPEGConversion;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c5_MPEGConversion;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c5_MPEGConversion;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c5_MPEGConversion;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c5_MPEGConversion;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c5_MPEGConversion;
  chartInstance->chartInfo.mdlStart = mdlStart_c5_MPEGConversion;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c5_MPEGConversion;
...
void c5_MPEGConversion_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c5_MPEGConversion(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c5_MPEGConversion(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c5_MPEGConversion(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c5_MPEGConversion_method_dispatcher.\n"
